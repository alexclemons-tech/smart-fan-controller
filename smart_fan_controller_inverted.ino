 ...
 * 
 * Variant:
 * - Inverted fan driver logic (2N2222 low-side switching stage driving IRF540N)
 */
...
void init_pwm() {
  ...
  uint8_t pwm_value = (FAN_SPEED_OFF * 255) / 100;
  if (FAN_DRIVER_INVERTED) {
...
void set_fan_speed(FanSpeed speed) {
  ...
  // Inverted driver stage: invert logical duty before output
  if (FAN_DRIVER_INVERTED) {
    pwm_value = 255 - pwm_value;
  }
